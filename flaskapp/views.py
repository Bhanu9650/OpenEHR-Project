...
def loginsucess():
    if request.method == "POST":
        ...
        result = (
            ...
            .filter(userdata.email == email, userdata.password == hashedPassword)
            .first()
        )
        if result is not None:
            ...
            return redirect(
                url_for(
                    "userHomePage", user_id=result.user_id, role=result.role.lower()
                )
            )
        else:
    ...
    if int(current_user) == int(user_id):
        if role == "doctor":
            prescription_data = (
                ...
                .filter(prescription.doctor_id == user_id)
                .all()
            )
            ...
            return render_template(
                "patient/home.html", data=role, data2=user_id, patient_data=patient_data
            )
        else:
        ...
        if patient_details is not None:
            ...
            if problem1 is not None:
                entry1 = (
                    ...
                    .update(
                        {
                            pastHistoryIllness.problem_name: request.form.get(
                                "problem_name"
                            ),
                            pastHistoryIllness.body_site: request.form.get("body_site"),
                            pastHistoryIllness.datetime_onset: request.form.get(
                                "datetime_onset"
                            ),
                            pastHistoryIllness.severity: request.form.get("severity"),
                            pastHistoryIllness.procedure_type: request.form.get(
                                "procedure_type"
                            ),
                        }
                    )
                    ...
                    .update(
                        {
                            ...
                            allergyIntolerance.verification_status: request.form.get(
                                "verification_status"
                            ),
                            allergyIntolerance.allergy_intol_type: request.form.get(
                                "allergy_intol_type"
                            ),
                            allergyIntolerance.comment: request.form.get("comment"),
                            allergyIntolerance.manifestation: request.form.get(
                                "manifestation"
                            ),
                        }
                    )
...
def doctorDiagnosisPage(current_user, doctor_id):
    ...
    if request.method == "GET":
        ...
        if patient_details is not None:
            problem1 = (
                db.session.query(problemList).filter_by(patient_id=patient_id).first()
            )
            if problem1 is not None:
                entry1 = (
                    db.session.query(problemList)
                    .filter_by(patient_id=patient_id)
                    .update(
                        {
                            problemList.problem_diag_name: request.form.get(
                                "problem_diag_name"
                            ),
                            problemList.body_site: request.form.get("body_site"),
                            problemList.datetime_onset: request.form.get(
                                "datetime_onset"
                            ),
                ...
                entry1 = problemList(
                    patient_id=patient_id,
                    problem_diag_name=problem_diag_name,
                    body_site=body_site,
                    datetime_onset=datetime_onset,
                    severity=severity,
                )
...
def patientSummary(current_user, doctor_id, patient_id):
    ...
    if data is None:
        data = (
            db.session.query(patient).filter(patient.patient_id == patient_id).first()
        )
        return render_template(
            "doctor/doctor_patient_summary.html",
            data1=[data],
            data="doctor",
            data2=doctor_id,
        )
...
def patientPresciptionPage(current_user, patient_id, prescription_id):
    ...
    total_prescription = (
        ...
        .join(
            doseDirection, prescription.prescription_id == doseDirection.prescription_id
        )
        .join(
            orderDetails, prescription.prescription_id == orderDetails.prescription_id
        )
...
def doctorPresciptionPage(current_user, doctor_id, prescription_id):
    ...
    return render_template(
        "doctor/doctor_prescription.html",
        data1=total_prescription,
        data="doctor",
        data2=doctor_id,
    )
